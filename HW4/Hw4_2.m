clear;close all;
ecg=load('F94051089_ecgdata.mat');
ecg = ecg.data(265:end);
fs = 250; %sampling rate
sze = length(ecg);
necg = ecg/max(ecg); % normalize the maximum value to unity
time = [1 : sze]/fs;
ecg = ecg - mean(ecg);
figure;subplot(3,2,[1,2]);axis tight;
plot(ecg(5000:10000));title('My raw ecg Signal');
...
subplot(323);plot(ecg_h(5000:10000));
title("Band Pass Filtered");axis tight;
% %% derivative filter H(z) = (1/8T)(-z^(-2) - 2z^(-1) + 2z + z^(2))
b = [1 2 0 -2 -1].*(1/8)*fs;  
ecg_d = filtfilt(b,1,ecg_h);
ecg_d = ecg_d/max(ecg_d);
subplot(324);plot(ecg_d(5000:10000));
title("Derivative Filterd");axis tight;
%% Squaring nonlinearly enhance the dominant peaks 
ecg_s = ecg_d.^2;
subplot(325);plot(ecg_s(5000:10000));title("Squared");axis tight;
%% Moving average Y(nt) = (1/N)[x(nT-(N - 1)T)+ x(nT - (N - 2)T)+...+x(nT)]
% N=30 for fs=200Hz
ecg_m=movmean(ecg_s,30);
subplot(326);plot(ecg_m(5000:10000));title("Moving Average");axis tight;
%%  Fiducial Marks
...
% figure;plot(ecg_m);
% title(['BPM= ',num2str(rate),' QRS width= ',num2str(QRS),' sec']);
% hold on;plot(loc,ecg_m(loc),'rx');axis tight;
...
rate2=Beat*60/t(end);
title(['BPM= ',num2str(rate2),' Width=',num2str(RR/fs),' sec']);
